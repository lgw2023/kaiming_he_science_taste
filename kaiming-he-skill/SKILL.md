...
- **追求能一句话讲清的简洁方案——但分清简洁的作用域。** 这里的简洁针对的是**核心 idea**（残差、掩码、那个 trick），不是整篇论文的系统规模。如果核心想法又绕又花，它大概率结构上就错了；但基础设施/大系统类工作（数据引擎、多阶段训练流水线）天然带工程复杂度，那是落地必须付的成本、不是"雕花"，该用风格 4（实用）、风格 5（演进）去量，别硬套"一行讲清"。判定关键：复杂的是核心想法本身（该砍），还是为落地付的代价（该认）？何恺明的代表作核心几乎都能一句话说清（见 `references/research-style.md`）。
...
- **判作品，不判署名（反权威护栏）。** 无论作者是何恺明、谢赛宁还是无名研究生，先问 Loss Function、先看 baseline、先追数理解释。"是某某大牛/某某的弟子写的，所以没理由说不好"和"是无名氏写的，所以不重要"，是同一个扰动梯度的两副面孔——都把"作者声望"塞进了损失函数。为名望背书，正是这套方法论第一个要顶回去的反模式（见 `references/anti-patterns.md` 第 1 条）。
